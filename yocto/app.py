...
SITE_BASE   = os.environ.get("SITE_BASE", "https://hiner.nyc").rstrip("/")
...
def format_dates(ci: date, co: date) -> str:
    ...
    # macOS/BSD may not support %-d, fall back if needed
    try:
...
def git_add_commit_push(paths: List[str]) -> Optional[str]:
    try:
        ...
        return (getattr(e, "stderr", None) or getattr(e, "stdout", None) or "git error")

# =========================
# HOTELS (SerpAPI + Render)
# =========================
...
def render_hotels_html(q: str, ci_s: str, co_s: str, data: Dict[str, Any]) -> str:
    ...
    city_title = titlecase_city(q) if q.lower() != "luxury" else "Luxury"
...
def std_aircraft(name: Optional[str]) -> str:
    if not name: return ""
    s = re.sub(r'\s+', ' ', str(name)).strip().lower()
    # Normalize many variants → standard names
    # Airbus
    if "a321" in s and "neo" in s: return "A321neo"
    if re.search(r'\ba321\b', s): return "A321"
    if re.search(r'\ba320\b', s) and "neo" in s: return "A320neo"
    if re.search(r'\ba320\b', s): return "A320"
    if "a319" in s: return "A319"
    if "a350-1000" in s or "a35k" in s: return "A350-1000"
    if "a350-900" in s or "a359" in s:  return "A350-900"
    if "a220-300" in s or "220-300" in s or "a223" in s: return "A220-300"
    if "a220-100" in s or "220-100" in s or "a221" in s: return "A220-100"
    # Boeing 737
    if "737" in s:
        if "max 9" in s or re.search(r'\b737-?9\b', s) or "b739" in s: return "B737-900"
        if "max 8" in s or re.search(r'\b737-?8\b', s) or "b738" in s: return "B737-800"
        if "700" in s or re.search(r'\b737-?7\b', s): return "B737-700"
        return "B737"
    # Boeing widebodies
    if "787-10" in s or "b78x" in s: return "B787-10"
    if "787-9"  in s or "b789" in s: return "B787-9"
    if "787-8"  in s or "b788" in s: return "B787-8"
    if "767-400" in s or "b764" in s: return "B767-400"
    if "767-300" in s or "b763" in s: return "B767-300"
    if "757-300" in s or "b753" in s: return "B757-300"
    if "757-200" in s or "b752" in s: return "B757-200"
    # RJ/Embraer/CRJ
    if "crj" in s:
        ...
        return "CRJ"
    m = re.search(r'(?:e|erj)[\s-]?(\d{2,3})', s)
    if m: return f"E{m.group(1)}"
    # Fallback: uppercase, keep hyphens
    return re.sub(r'[^a-zA-Z0-9-]+', '', name).upper()

# Airline → plane strip (overlay) — files live in /yocto/logos/
PLANE_STRIPS = {
...
def plane_strip_url(airline: Optional[str]) -> Optional[str]:
    s = (airline or "").lower()
    for key, fname in PLANE_STRIPS.items():
        if key in s:
            return f"{SITE_BASE}/yocto/logos/{fname}"
    return None
...
def flights_from_json(data: Dict[str, Any]) -> List[Dict[str, Any]]:
    ...
    for bucket in ("best_flights", "other_flights", "results", "flights", "items"):
        arr = data.get(bucket)
        ...
        for it in src:
            ...
            segs = it.get("flights") or it.get("segments") or []
            if not segs: continue
            first, last = segs[0], segs[-1]
            dep_code = (first.get("departure_airport") or {}).get("id") or (first.get("from") or {}).get("id") or ""
            arr_code = (last.get("arrival_airport") or {}).get("id") or (last.get("to") or {}).get("id") or ""
            dep_time = to_24h((first.get("departure_airport") or {}).get("time") or first.get("departure_time") or first.get("depart_time"))
            arr_time = to_24h((last.get("arrival_airport") or {}).get("time") or last.get("arrival_time") or last.get("arrive_time"))
            airline  = first.get("airline") or it.get("airline") or ""
            total    = it.get("total_duration") or it.get("duration")
            lays = []
            for l in it.get("layovers") or []:
                ...
                lays.append({
                    "id": (l.get("id") or l.get("name") or l.get("airport") or "").upper(),
                    "dur": mm_to_hhmm(lay_min) if lay_min else None
                })
            legs = []
            for seg in segs:
                legs.append({
                    "dep": ((seg.get("departure_airport") or seg.get("from") or {}).get("id") or seg.get("from") or ""),
                    "arr": ((seg.get("arrival_airport")   or seg.get("to")   or {}).get("id") or seg.get("to") or ""),
                    "dep_time": to_24h((seg.get("departure_airport") or {}).get("time") or seg.get("departure_time") or seg.get("depart_time")),
                    "arr_time": to_24h((seg.get("arrival_airport") or {}).get("time") or seg.get("arrival_time") or seg.get("arrive_time")),
                    "plane": std_aircraft(seg.get("airplane") or seg.get("aircraft") or seg.get("equipment")),
                    "num": seg.get("flight_number") or seg.get("number") or "",
                })
            out.append({
                "price": it.get("price"),
...
def render_flights_html(dep_disp: str, arr_disp: str, date_str: str, data: Dict[str, Any], class_disp: str = "first") -> str:
    ...
    # header date, tolerant of BSD strftime
    try:
        subtitle = datetime.strptime(date_str, "%Y-%m-%d").strftime("%a, %b %-d, %Y")
    except Exception:
        try:
            subtitle = datetime.strptime(date_str, "%Y-%m-%d").strftime("%a, %b %d, %Y")
        except Exception:
    ...
    for f in items:
        ...
        for lg in f.get("legs") or []:
            ...
            <div class="legline">
              ...
              <div class="cell plane">{esc(lg['plane'])} {esc(lg['num'])}</div>
            </div>
            """
            leg_rows.append(row)
        legs_html = "\n".join(leg_rows)
...
:root{{ --blue:#27308D; --gold:#FFC107; }}
*{{box-sizing:border-box}}
body{{margin:0;background:#93c5fd;color:#000;font-family:'AcuminCond', system-ui, -apple-system, Arial, sans-serif}}
...
.fc{{
  position:relative; border-radius:28px; overflow:hidden; margin:22px 0;
...
.fc .codes{{
  ...
  font-weight:900; letter-spacing:2px; color:var(--blue);
  font-size: clamp(44px, 16vw, 92px); line-height:.9;
  display:flex; gap:14px; align-items:flex-start;
  pointer-events:none; user-select:none;
}}
.fc .bar{{
  position:absolute; left:0; right:0; top:0; height:34px; z-index:3;
  background:var(--gold); color:#111; display:flex; align-items:center; justify-content:center;
  font-family:'AcuminCond'; font-weight:800; letter-spacing:.2px;
}}
.fc .plane{{
...
.fc .pane{{
  ...
  z-index:3; /* under plane so plane clips codes/pane */
  backdrop-filter: saturate(120%) blur(1.5px);
}}
.fc .grid{{ display:block }}
.fc .legline{{
  display:grid;
  grid-template-columns:64px 64px 56px 56px minmax(150px, 1fr);
...
.fc .lay{{ margin-top:8px; font-family:'AcuminCond'; font-weight:800; }}
...
def run_hotels():
    ...
    data = serpapi_hotels(q, ci_s, co_s)
    html_out = render_hotels_html(q, ci_s, co_s, data)
